...
const emit = defineEmits(['run', 'delete', 'refresh', 'run-batch'])

// Track selected account IDs
const selectedIds = ref([])

// Watch accounts list to remove selected IDs that no longer exist
watch(() => props.accounts, (newAccs) => {
  const existingIds = newAccs.map(a => a.id)
...
const handleBatchRun = () => {
  ...
  emit('run-batch', selectedIds.value)
  selectedIds.value = []
}
</script>
  ...
  <section class="bg-zinc-900/60 border border-zinc-900 rounded-2xl p-5 shadow-xl flex-grow flex flex-col overflow-hidden backdrop-blur-sm">
    <h2 class="text-sm font-semibold uppercase tracking-wider text-zinc-400 mb-4 flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        <label v-if="accounts.length > 0" class="flex items-center gap-1.5 text-[11px] text-zinc-500 hover:text-zinc-300 cursor-pointer select-none">
          <input
            type="checkbox"
            :checked="isAllSelected"
            @change="toggleSelectAll"
            :disabled="isRunning"
            class="rounded border-zinc-800 bg-zinc-950 text-purple-600 focus:ring-purple-500 focus:ring-offset-0 disabled:opacity-40"
          />
          Chọn hết
        </label>
      ...
      >
        <div class="flex items-center gap-2 min-w-0">
          ...
          <input
            ...
            v-model="selectedIds"
            :disabled="isRunning"
            class="rounded border-zinc-800 bg-zinc-950 text-purple-600 focus:ring-purple-500 focus:ring-offset-0 disabled:opacity-40 mr-1 shrink-0"
          />
          
          <div class="flex items-center gap-2 min-w-0">
            <!-- Platform Indicator -->
            <span class="text-sm px-1.5 py-0.5 bg-zinc-900 rounded">{{ acc.platform === 'facebook' ? '📘' : acc.platform === 'youtube' ? '📺' : acc.platform === 'tiktok' ? '🎵' : '🐦' }}</span>
            <div class="min-w-0">
              <p class="text-xs font-semibold truncate text-zinc-200" :title="acc.username">{{ acc.username }}</p>
              <div class="flex items-center gap-1.5 mt-0.5">
                <span :class="getStatusBadgeClass(acc.status)" class="px-1.5 py-0.2 rounded text-[9px] font-medium tracking-wide">
                  {{ acc.status }}
                </span>
                <span v-if="acc.last_checked_at" class="text-[9px] text-zinc-500">
        ...
        <div class="flex items-center gap-1.5 shrink-0">
          <button
            ...
            class="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold transition-all duration-200"
          >
            <svg v-if="isRunning && activeAccountRunning?.id === acc.id" class="animate-spin h-3 w-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
              <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="w-3 h-3">
          ...
          <button
            ...
            :disabled="isRunning"
            class="p-1.5 rounded-lg bg-zinc-950 hover:bg-red-500/10 text-zinc-500 hover:text-red-400 border border-zinc-900 transition-colors"
          >
    ...
    <div
      ...
      class="mt-4 pt-3 border-t border-zinc-800/80 flex items-center justify-between animate-fadeIn"
    >
      <span class="text-xs text-zinc-400 font-medium">
        Đã chọn <strong class="text-purple-400 font-semibold">{{ selectedIds.length }}</strong> tài khoản
      </span>
      <button
        @click="handleBatchRun"
        :disabled="isRunning"
        class="bg-purple-600 hover:bg-purple-500 text-white px-4 py-2 rounded-xl text-xs font-semibold shadow-lg shadow-purple-900/30 transition-all duration-200 flex items-center gap-1.5 disabled:opacity-50"
      >
